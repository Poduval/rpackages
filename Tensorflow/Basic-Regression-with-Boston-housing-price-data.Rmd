---
title: "Basic Regression with Boston housing price data"
...
It has 506 total examples that are split between 404 training examples and 102 test examples:

```{r}
boston_housing <- dataset_boston_housing()
# str(boston_housing, 2)

c(train_data, train_labels) %<-% boston_housing$train
c(test_data, test_labels) %<-% boston_housing$test

sprintf("#Training entries: %i,  # labels: %i", length(train_data), length(train_labels))
```
...
-   If there is not much training data, prefer a small network with few hidden layers to avoid over fitting.

-   Early stopping is a useful technique to prevent over fitting.
